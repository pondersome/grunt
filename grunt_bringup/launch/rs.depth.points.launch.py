...
configurable_parameters = [{'name': 'camera_name',                  'default': 'cam_live', 'description': 'camera unique name'},
                           ...
                           {'name': 'pointcloud.enable',            'default': 'false', 'description': 'Default off: depth/color/points is the heaviest topic (~196 MB/s at the upstream defaults) and is fully derivable post-hoc from depth + camera_info. Enable only when a live point cloud is required (Foxglove 3D viz, manipulation pipelines).'},
